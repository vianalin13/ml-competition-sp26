...
FEATURE_COLUMNS = [
    ...
    # pruning some of these features because CV showed the full liquidity group underperformed
    "amount_ma_20d",  # 20-day moving average of amount (trading value), captures liquidity
    # "volume_ma_20d",  # 20-day moving average of volume, also captures liquidity
    # "volume_std_20d",  # 20-day rolling std of volume, captures volume volatility/spikes
    # "turnover_std_20d",  # 20-day rolling std of turnover, captures turnover volatility/spikes
    "turnover_z_20d",  # (turnover - turnover_ma_20d) / turnover_std_20d, captures turnover spikes
    "zero_volume_5d",  # count of zero-volume days in past 5 days, captures recent illiquidity
    # "amihud_20d",  # 20-day Amihud illiquidity ratio = mean(|ret_1d| / amount), captures price impact of trading
]
...
def _per_stock_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # df["volume_ma_20d"] =  vol_mean
    # df["volume_std_20d"] = vol_std

    # amount/traded value
    if "amount" in df.columns:
        amount = df["amount"].astype(float)
        df["amount_ma_20d"] = amount.rolling(20).mean()
    else:
        amount = pd.Series(np.nan, index=df.index)
        df["amount_ma_20d"] = np.nan
    ...
    if "turnover" in df.columns:
        ...
        df["turnover_ma_20d"] = turnover_ma
        # df["turnover_std_20d"] = turnover_std

        turnover_std_safe = turnover_std.replace(0, np.nan)
        df["turnover_z_20d"] = (turnover - turnover_ma) / turnover_std_safe
    else:
        df["turnover_ma_20d"] = np.nan
        # df["turnover_std_20d"] = np.nan
        df["turnover_z_20d"] = np.nan

    # recent no-trade/suspension proxy
    df["zero_volume_5d"] = (vol <= 0).astype(float).rolling(5).sum()

    # Amihud illiquidity: abs return per unit traded amount
    # Higher = price moves more for same traded value = less liquid
    # daily_abs_ret = df["ret_1d"].abs()
    # if "amount" in df.columns:
    #     df["amihud_20d"] = (
    #           daily_abs_ret / amount.replace(0, np.nan)
    #       ).rolling(20).mean()
    # else:
    ...
    # target: 5-day forward return on forward-adjusted close
